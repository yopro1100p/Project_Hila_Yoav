...
def perform_statistical_tests(predictable_file, controller_file):
    ...
    for metric, result in test_results.items():
        print(
            f"{metric} - {result['test_type']} statistic: {result['statistic']}, p-value: {result['p_value']}, {result['significance']}")
    ...
    output_file_name = os.path.join(output_folder,
                                    "statistical_test_results_" + predictable_file.replace('predictable', ''))
