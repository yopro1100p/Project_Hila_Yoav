...
# File explanation
"""
This script processes and compares data from two different neural recordings 
obtained from 120 electrodes. It extracts features such as the number of spikes,
bursts, and spike rates from each electrode for both a baseline recording and 
a stimulus recording. After calculating these features for both recordings, 
it computes the differences between the baseline and stimulus for each electrode. 
The script also identifies if the data is comparable between the two recordings
and marks whether there is a positive or negative difference in spike count. 
Finally, it calculates the averages and standard deviations of the key metrics 
across all electrodes and writes the results to an Excel file, ensuring that the
data is only written when the file doesn't exist or when there are updates.
"""
...
def recording_comparison(file_path_compare1, file_path_compare2):
    ...
    if match:
        ...
        raise ValueError("File path doesn't match the expected pattern.")

    # Define the output file name with the extracted base name
    ...
    for electrode_num in range(120):
        ...
        analyzer1 = ChannelAnalyzer(file_path_compare1, electrode_num)
        analyzer2 = ChannelAnalyzer(file_path_compare2, electrode_num)
        ...
        if (analyzer1.comparable == False) and (analyzer2.comparable == False):
            ...
            if analyzer2.num_of_spikes - analyzer1.num_of_spikes > 0:
                ...
                if analyzer2.num_of_spikes - analyzer1.num_of_spikes < 0:
                    df.at[electrode_num, 'negative_diff'] = 'true'
            if analyzer2.num_of_spikes != 0:
                df.at[electrode_num, 'num_of_spikes_diff_precent'] = (
                                                                                 analyzer2.num_of_spikes - analyzer1.num_of_spikes) * 100 / analyzer2.num_of_spikes
            ...
            if analyzer2.Average_Spikes != 0:
                df.at[electrode_num, 'average_absolute_spikes_diff_precent'] = (
                                                                                           analyzer2.Average_Spikes - analyzer1.Average_Spikes) * 100 / analyzer2.Average_Spikes
            df.at[electrode_num, 'Spikes_rate_diff'] = analyzer2.Spikes_rate - analyzer1.Spikes_rate
            if analyzer2.Spikes_rate != 0:
                df.at[electrode_num, 'Spikes_rate_diff_precent'] = (
                                                                               analyzer2.Spikes_rate - analyzer1.Spikes_rate) * 100 / analyzer2.Spikes_rate
            if analyzer2.Num_Of_Bursts != 0:
                df.at[electrode_num, 'num_of_bursts_diff'] = analyzer2.Num_Of_Bursts - analyzer1.Num_Of_Bursts
                df.at[electrode_num, 'num_of_bursts_diff_precent'] = (
                                                                                 analyzer2.Num_Of_Bursts - analyzer1.Num_Of_Bursts) * 100 / analyzer2.Num_Of_Bursts
                df.at[electrode_num, 'spikes_per_bursts_diff'] = analyzer2.spikes_per_burst - analyzer1.spikes_per_burst
                df.at[electrode_num, 'spikes_per_bursts_diff_precent'] = (
                                                                                     analyzer2.spikes_per_burst - analyzer1.spikes_per_burst) * 100 / analyzer2.spikes_per_burst
